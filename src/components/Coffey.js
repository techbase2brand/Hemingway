...
export default function Coffey() {
  ...
  const countSyllables = (word) => {
    ...
    word = word.replace(/(?:e$|ed|es|[^aeiouy]e[^aeiouy])/ig, '');  // Remove silent e's and e's at the end of words
  ...
  const format = () => {
    ...
    let sentences = inputText.split('\n').map(p => getSentenceFromParagraph(p)).flat().length;
    let newData = {
    ...
    const gradeLevel = calculateGradeLevel(newData.wordCount, sentences, inputText);
